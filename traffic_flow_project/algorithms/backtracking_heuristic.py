...
class HeuristicBacktrackingSolver:
    """Heuristic - Backtracking tự cài đặt từ đầu.

    Trước khi duyệt, sắp xếp path theo cost tăng, capacity giảm, độ dài tăng.
    Kết hợp bound nhẹ để tìm nghiệm tốt sớm và cắt bớt nhánh.
    """
    def __init__(self, graph_model:GraphModel, max_paths:int=20, max_logs:int=500):
        # Khởi tạo dữ liệu và sắp xếp path theo heuristic.
    ...
    def log(self,msg:str):
        # Ghi log các bước tìm kiếm heuristic.
    ...
    def max_feasible(self,p:PathData,usage:Dict[Tuple[str,str],int],remaining:int)->int:
        # Tính flow tối đa còn có thể gán cho path.
    ...
    def upper_bound(self,i:int,flow:int,usage:Dict[Tuple[str,str],int])->int:
        # Ước lượng cận trên flow còn có thể đạt.
    ...
    def solve(self)->SolutionSummary:
        # Điều khiển quá trình giải bằng Heuristic Ordering.
        started=perf_counter(); assign=[0]*len(self.paths); usage:Dict[Tuple[str,str],int]={}
        if not self.paths: return SolutionSummary('Heuristic',0,0.0,0.0,{},[],{"log_text":"Không có path.","path_count":0})
        ...
        return SolutionSummary('Heuristic',int(ev['total_flow']),float(ev['total_cost']),runtime,{k:int(v) for k,v in ev['edge_usage'].items()},pf,{"states_visited":self.stats.states_visited,"recursive_calls":self.stats.recursive_calls,"pruned_branches":self.stats.pruned_branches,"path_count":len(self.paths),"log_text":"\n".join(self.logs)})
    def _backtrack(self,i:int,flow:int,cost:float,assign:List[int],usage:Dict[Tuple[str,str],int]):
        # Đệ quy theo thứ tự path đã ưu tiên.
