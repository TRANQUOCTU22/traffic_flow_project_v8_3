...
class FullOptimizationBacktrackingSolver:
    """Full Optimization tự cài đặt từ đầu.

    Kết hợp Forward Checking + Branch and Bound + Heuristic Ordering + Constraint Propagation.
    Đây là bản Backtracking mạnh nhất để làm nghiệm tham chiếu.
    """
    def __init__(self, graph_model:GraphModel, max_paths:int=20, max_logs:int=500):
        # Khởi tạo dữ liệu cho Full Optimization.
        self.g=graph_model; self.paths=self.g.simple_paths(max_paths=max_paths); self.paths.sort(key=lambda p:(p.cost,-p.capacity,len(p.nodes))); self.caps=self.g.edge_capacity_map()
    ...
    def log(self,msg:str):
        # Ghi log quá trình tối ưu tổng hợp.
    ...
    def max_feasible(self,p:PathData,usage:Dict[Tuple[str,str],int],remaining:int)->int:
        # Tính flow khả thi sau lan truyền ràng buộc.
        if remaining<=0: return 0
        return max(0,min([remaining]+[self.caps[e]-usage.get(e,0) for e in p.edges]))
    def upper_bound(self,i:int,flow:int,usage:Dict[Tuple[str,str],int])->int:
        # Ước lượng cận trên để hỗ trợ cắt nhánh.
        rem=self.g.demand-flow
        if rem<=0: return flow
        add=0
        for p in self.paths[i:]:
            add+=self.max_feasible(p,usage,rem-add)
            if add>=rem: break
        return flow+min(rem,add)
    def solve(self)->SolutionSummary:
        # Điều khiển phiên bản Backtracking tối ưu nhất.
        started=perf_counter(); assign=[0]*len(self.paths); usage:Dict[Tuple[str,str],int]={}
        if not self.paths: return SolutionSummary('Full Optimization',0,0.0,0.0,{},[],{"log_text":"Không có path.","path_count":0})
        ...
        return SolutionSummary('Full Optimization',int(ev['total_flow']),float(ev['total_cost']),runtime,{k:int(v) for k,v in ev['edge_usage'].items()},pf,{"states_visited":self.stats.states_visited,"recursive_calls":self.stats.recursive_calls,"pruned_branches":self.stats.pruned_branches,"path_count":len(self.paths),"log_text":"\n".join(self.logs)})
    def _backtrack(self,i:int,flow:int,cost:float,assign:List[int],usage:Dict[Tuple[str,str],int]):
        # Đệ quy kết hợp forward, pruning, heuristic và constraint.
        self.stats.recursive_calls+=1
        ub=self.upper_bound(i,flow,usage)
        if ub<self.best_flow or (ub==self.best_flow and cost>=self.best_cost):
            self.stats.pruned_branches+=1; return
