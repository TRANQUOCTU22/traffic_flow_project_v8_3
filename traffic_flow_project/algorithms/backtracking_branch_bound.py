...
class BranchBoundBacktrackingSolver:
    """BnB - Branch and Bound tự cài đặt từ đầu.

    Dùng upper bound lạc quan: current_flow + khả năng tối đa còn lại.
    Nếu bound không thể vượt nghiệm tốt nhất thì cắt nhánh.
    """
    def __init__(self, graph_model:GraphModel, max_paths:int=20, max_logs:int=500):
        # Khởi tạo dữ liệu cho Branch and Bound.
    ...
    def log(self,msg:str):
        # Ghi log các nhánh được xét hoặc cắt.
    ...
    def solve(self)->SolutionSummary:
        # Điều khiển quá trình giải bằng Branch and Bound.
        started=perf_counter(); assign=[0]*len(self.paths); usage:Dict[Tuple[str,str],int]={}
        if not self.paths: return SolutionSummary('BnB',0,0.0,0.0,{},[],{"log_text":"Không có path.","path_count":0})
        ...
        return SolutionSummary('BnB',int(ev['total_flow']),float(ev['total_cost']),runtime,{k:int(v) for k,v in ev['edge_usage'].items()},pf,{"states_visited":self.stats.states_visited,"recursive_calls":self.stats.recursive_calls,"pruned_branches":self.stats.pruned_branches,"path_count":len(self.paths),"log_text":"\n".join(self.logs)})
    def _backtrack(self,i:int,flow:int,cost:float,assign:List[int],usage:Dict[Tuple[str,str],int]):
        # Đệ quy tìm nghiệm và cắt nhánh bằng cận trên.
