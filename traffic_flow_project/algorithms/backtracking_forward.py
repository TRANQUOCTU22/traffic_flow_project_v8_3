...
class ForwardCheckingBacktrackingSolver:
    """FC - Forward Checking tự cài đặt từ đầu.

    Mỗi khi gán flow cho một path, thuật toán kiểm tra ngay demand và capacity.
    Nhánh nào vi phạm bị loại trước khi đi sâu hơn.
    """
    def __init__(self, graph_model:GraphModel, max_paths:int=20, max_logs:int=500):
        # Khởi tạo dữ liệu cho Forward Checking.
    ...
    def solve(self)->SolutionSummary:
        # Điều khiển quá trình giải bằng Forward Checking.
        started=perf_counter(); assign=[0]*len(self.paths); usage:Dict[Tuple[str,str],int]={}
        if not self.paths: return SolutionSummary('FC',0,0.0,0.0,{},[],{"log_text":"Không có path.","path_count":0})
        ...
        return SolutionSummary('FC',int(ev['total_flow']),float(ev['total_cost']),runtime,{k:int(v) for k,v in ev['edge_usage'].items()},pf,{"states_visited":self.stats.states_visited,"recursive_calls":self.stats.recursive_calls,"pruned_branches":self.stats.pruned_branches,"path_count":len(self.paths),"log_text":"\n".join(self.logs)})
    def _backtrack(self,i:int,flow:int,cost:float,assign:List[int],usage:Dict[Tuple[str,str],int]):
        # Đệ quy gán flow và loại nhánh vi phạm sớm.
